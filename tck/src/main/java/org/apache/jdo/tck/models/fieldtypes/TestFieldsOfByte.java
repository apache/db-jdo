 ...
 * <B>Assertion ID:</B> A6.4.3-11. <br>
 * <B>Assertion Description: </B> JDO implementations must support fields of the immutable object
 * class <code>java.lang.Byte</code>, and may choose to support them as Second Class Objects or
 * First Class Objects.
 */
public class TestFieldsOfByte extends JDO_Test {

  /** */
  private static final String ASSERTION_FAILED = "Assertion A6.4.3-11 (TestFieldsOfByte) failed: ";
  ...
  private void checkValues(Object oid, Byte startValue) {
    ...
    int n = pi.getLength();
    for (i = 0; i < n; ++i) {
      if (!FieldsOfByte.isPersistent[i]) continue;
      Byte val = pi.get(i);
      if (!val.equals(startValue)) {
        fail(
            ASSERTION_FAILED,
            "Incorrect value for "
                + FieldsOfByte.fieldSpecs[i]
                + ", expected value "
                + startValue.toString()
                + ", value is "
                + val);
