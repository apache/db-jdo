...
abstract class AbstractPersistenceCapableTest extends JDO_Test {
  ...
  protected boolean isTestToBePerformed() {
    ...
    if (!isBinaryCompatibilitySupported()) {
      printUnsupportedOptionalFeatureNotTested(testName, "javax.jdo.option.BinaryCompatibility");
    } else if (!isApplicationIdentitySupported()) {
      printUnsupportedOptionalFeatureNotTested(testName, "javax.jdo.option.ApplicationIdentity");
    }
