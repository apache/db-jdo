...
/**
 * <B>Title:</B> Rollback <br>
 * <B>Keywords:</B> transactions <br>
 * <B>Assertion ID:</B> A13.4.4-2. <br>
 * <B>Assertion Description: </B> The <code>rollback</code> method performs the following
 * operations:
 *
 * <UL>
 *   <LI>rolls back changes made in this transaction from the data store;
...
public class Rollback extends JDO_Test implements Synchronization {
  ...
  void runTestRollback(PersistenceManager pm) {
    ...
    try {
      ...
      tx.commit();
      if (x != 1) {
        fail(ASSERTION_FAILED, "tx.rollback should rollback change of ip1.x, expected 1, got " + x);
      }
