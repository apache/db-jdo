 ...
 * <B>Keywords:</B> <br>
 * <B>Assertion IDs:</B> A12.5.7-23 <br>
 ...
 * transient instance takes its values as of the beginning of the transaction, if the call to
 * makeTransactional was made prior to the beginning of the current transaction.
 */
public class MakeTransactionalPriorToTransactionRolledback extends PersistenceManagerTest {
  ...
  public void runTestMakeTransactionalPriorToTransactionRolledback(PersistenceManager pm) {
    Transaction tx = pm.currentTransaction();
    try {
      if (debug) logger.debug(" ** in runTestMakeTransactionalPriorToTransactionRolledback()");
      ...
      if (p1.getY().intValue() != 300) {
        fail(
            ASSERTION_FAILED,
            "wrong value of p1.y, expected: 300, actual: " + p1.getY().intValue());
      }
    } finally {
  ...
  private void runTestMakeTransactionalPriorToTransactionRolledbackAll1(PersistenceManager pm) {
    Transaction tx = pm.currentTransaction();
    try {
      if (debug) logger.debug(" ** in runTestMakeTransactionalPriorToTransactionRolledbackAll1()");
      ...
      if (p2.getY().intValue() != 300) {
        fail(
            ASSERTION_FAILED,
            "wrong value of p2.y, expected: 300, actual: " + p2.getY().intValue());
      }
      if (p3.getY().intValue() != 300) {
        fail(
            ASSERTION_FAILED,
            "wrong value of p3.y, expected: 300, actual: " + p3.getY().intValue());
      }
    } finally {
      if ((tx != null) && tx.isActive()) tx.rollback();
    }
   ...
   */
  public void runTestMakeTransactionalPriorToTransactionRolledbackAll2(PersistenceManager pm) {
    Transaction tx = pm.currentTransaction();
    try {
      if (debug) logger.debug(" ** in runTestMakeTransactionalPriorToTransactionRolledbackAll2()");
      p2.setY(Integer.valueOf(50));
      p3.setY(Integer.valueOf(50));
      tx.begin();
      p2.setY(Integer.valueOf(100));
