...
public class ClassGenerator {
  ...
  void generate() throws Exception {
    ...
    for (int i = 0; i < fieldTypes.length; i++) {
      String classFile =
          (isPrimitive(fieldTypes[i]) ? "FieldsOfPrimitive" : "FieldsOf") + fieldTypes[i];
      ...
      for (int j = 0; j < accessSpecifiers.length; j++) {
        for (int k = 0; k < fieldModifiers.length; k++) {
          for (int l = 0; l < xmlPersistenceModifiers.length; l++) {
            ...
            if ((fieldModifiers[k].indexOf("static") >= 0
                    || fieldModifiers[k].indexOf("final") >= 0)
                && !xmlPersistenceModifiers[l].equals("")) continue;
            ...
            */
            for (int m = 0; m < xmlEmbeddedModifiers.length; m++) {
              ...
              boolean fieldIsPersistent =
                  !(fieldModifiers[k].indexOf("static") >= 0
                      || fieldModifiers[k].indexOf("final") >= 0
                      || xmlPersistenceModifiers[l].indexOf("none") >= 0
                      || xmlPersistenceModifiers[l].indexOf("transactional") >= 0
                      || (fieldModifiers[k].indexOf("transient") >= 0
                          && xmlPersistenceModifiers[l].indexOf("persistent") == -1));

              if (!xmlEmbeddedModifiers[m].equals("") && !fieldIsPersistent) continue;
              ...
              buildFieldSpecs(
                  xmlPersistenceModifiers[l].replace('"', ' ')
                      + " "
              ...
              if (isFinal[fieldCounter - 1]) sb.append(getInitializerForFinalTypes(fieldTypes[i]));
    ...
    for (int i = 0; i < collectionTypes.length; i++) {
      ...
      } else // Array and the other collections
      {
        ...
        for (int j = 0; j < elementTypes.length; j++) {
          if (elementTypes[j].equals("Locale") && collectionTypes[i].equals("TreeSet")) continue;
            ...
            if (collectionTypes[i].equals("Array")) {
              if (!embeddedElements[k].equals("")) {
                fieldNames[fieldCounter] =
                    collectionTypes[i] + "Of" + elementTypes[j] + fieldCounter;
        ...
        writeMethodGet(
            pw,
            collectionTypes[i].equals("Array") ? "Object [] " : collectionTypes[i],
            fieldNames,
            fieldCounter);
        if (collectionTypes[i].equals("Array"))
          writeMethodSetForArray(pw, "Object [] ", fieldNames, fieldCounter);
        else writeMethodSet(pw, collectionTypes[i], fieldNames, fieldCounter);
      ...
      for (int j = 0; j < embeddedKeys.length; j++) {
        for (int k = 0; k < valueTypes.length; k++) {
          for (int l = 0; l < embeddedValues.length; l++) {
            if (keyTypes[i].equals("")
                && embeddedKeys[j].equals("")
                && valueTypes[k].equals("")
                && embeddedValues[l].equals("")) continue;
            fieldNames[fieldCounter] =
                mapName + "Of" + keyTypes[i] + "_" + valueTypes[k] + fieldCounter;
