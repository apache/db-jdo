 ...
 * <B>Assertion ID:</B> A10.4-2. <br>
 * <B>Assertion Description: </B> Access to field values within a call to <code>jdoPreDelete()
 * </code> of a class implementing <code>InstanceCallbacks</code> are valid.
 */
...
public class AccessingFieldsInPredelete extends TestParts {
  ...
  public void test() {
    ...
    // check that secondaryObj had jdoPreDelete() called on it and provided proper access to its
    // attributes.
    checkFieldValues(
        ASSERTION_FAILED,
        "jdoPreDelete attribute access:  ",
    ...
    try {
      ...
      fail(
          ASSERTION_FAILED,
          "secondaryObj should have been deleted but getObjectById() on its Id succeeded.");
    } catch (JDOUserException e) {
      // expected one of these exceptions
    } catch (JDODataStoreException e) {
      // expected one of these exceptions
    ...
    // check that first added member of Set had jdoPreDelete() called on it and provided proper
    ...
    try {
      ...
      fail(
          ASSERTION_FAILED,
          "First added member of Set primaryObj.children should have been deleted but getObjectById() on its Id succeeded.");
    } catch (JDOUserException e) {
      // expected one of these exceptions
    } catch (JDODataStoreException e) {
      // expected one of these exceptions
    ...
    // check that the second added member of Set did not have jdoPreDelete() called on it and it was
    // not deleted.
    ...
    try {
      ...
      fail(
          ASSERTION_FAILED,
          "Second added member of Set primaryObj.children should exist but getObjectById() got JDOUserException "
              + e);
    } catch (JDODataStoreException e) {
      fail(
          ASSERTION_FAILED,
          "Second added member of Set primaryObj.children should exist but getObjectById() got JDODataStoreException "
              + e);
    }
    t.rollback();
    pm.close();
