 ...
 * <B>Assertion ID:</B> A8.1-1. <br>
 * <B>Assertion Description: </B> The jdohelper.getPersistenceManager method returns associated
 * persistence manager if the object parameter is not null and implements persistenceCapable.
...
public class GetPersistenceManager extends JDO_Test {
  ...
  public void testGetPersistenceManager() {
    ...
    if (pm1 == null)
      fail(
          ASSERTION_FAILED, "JDOHelper.getPersistenceManager returns null for persistent instance");
    if (pm1 != pm)
      fail(
          ASSERTION_FAILED,
          "JDOHelper.getPersistenceManager returns wrong pm " + pm1 + ", expected " + pm);
    pm.close();
    pm = null;
  }
}
