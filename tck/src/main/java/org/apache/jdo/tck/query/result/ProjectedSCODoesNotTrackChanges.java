 ...
 * <B>Keywords:</B> embedded query <br>
 * <B>Assertion ID:</B> A14.6.9-9. <br>
 * <B>Assertion Description: </B> If an SCO field is in the result, the projected field is not owned
 * by any persistent instance, and modifying the SCO value has no effect on any persistent instance.
 * If an FCO field is in the result, the projected field is a persistent instance, and modifications
 * made to the instance are reflected as changes to the datastore per transaction requirements.
 */
public class ProjectedSCODoesNotTrackChanges extends JDO_Test {
  ...
  private Company getPersistentNewInstance(long companyid) {
    Company obj =
        new Company(companyid, "MyCompany", expectedDate, new Address(0, "", "", "", "", ""));
  ...
  public void testEmbeddedField() {
    ...
    pm.currentTransaction().begin();
    Query<Company> query = pm.newQuery(Company.class, "name.startsWith(\"MyCompany\")");
    ...
    if (JDOHelper.isDirty(comp)) {
      appendMessage(
          "Expected Company instance not to be dirty; "
              + "actual state is "
              + getStateOfInstance(comp));
    }
    pm.currentTransaction().commit();
    ...
    if (actualZip.equals(testZip)) {
      appendMessage(
          "Expected projected field value is " + testZip + "; actual value is " + actualZip);
    }
  ...
  public void testDateField() {
    ...
    if (!actualDate.equals(expectedDate)) {
      appendMessage(
          "Expected projected field value is "
              + expectedDate
              + "; actual value is "
              + actualDate
              + "; modified retrieved value is "
              + retrievedDate);
    }
