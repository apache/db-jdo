...
/**
 * <B>Title:</B> Support of field type BigDecimal. <br>
 * <B>Keywords:</B> model <br>
 * <B>Assertion ID:</B> A6.4.3-19. <br>
 * <B>Assertion Description: </B> JDO implementations must support fields of the immutable object
 * class <code>java.math.BigDecimal</code>, and may choose to support them as Second Class Objects
    ...
    for (i = 0, value = startValue; i < n; ++i) {
      ...
      if (val.compareTo(value) != 0) {
        fail(
            ASSERTION_FAILED,
            "Incorrect value for "
                + FieldsOfBigDecimal.fieldSpecs[i]
                + ", expected value "
                + value.toString()
                + ", value is "
                + val);
