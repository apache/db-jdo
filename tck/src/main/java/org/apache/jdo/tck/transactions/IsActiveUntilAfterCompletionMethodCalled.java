...
/**
 * <B>Title:</B> Is Active Until After Completion Method Called <br>
 * <B>Keywords:</B> transactions <br>
 * <B>Assertion ID:</B> A13.4.1-3. <br>
 * <B>Assertion Description: </B> Transaction.isActive returns true after the transaction has been
 * started, until the afterCompletion synchronization method is called
 */
 ...
 */
public class IsActiveUntilAfterCompletionMethodCalled extends JDO_Test implements Synchronization {
  ...
  public void beforeCompletion() {
    try {
      if (!tx.isActive()) {
        fail(ASSERTION_FAILED, "tx.isActive returns false in beforeCompletion.");
      }
    } catch (Exception ex) {
      fail(
          ASSERTION_FAILED,
  ...
  public void afterCompletion(int status) {
    try {
      if (tx.isActive()) {
        fail(ASSERTION_FAILED, "tx.isActive returns true in afterCompletion.");
      }
    } catch (Exception ex) {
      fail(
          ASSERTION_FAILED,
