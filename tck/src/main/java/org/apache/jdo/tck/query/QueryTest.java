...
public abstract class QueryTest extends AbstractReaderTest {
  ...
  /**
   * Returns a list of persistent company model instances instances for beans names in the given
   * argument.
   *
  ...
  protected List getPersistentCompanyModelInstancesAsList(String[] beanNames) {
    return new ArrayList(Arrays.asList(getPersistentCompanyModelInstances(beanNames)));
  }

  /**
   * Returns a list of transient company model instances instances for beans names in the given
   * argument.
   *
   * @param beanNames the bean names of company model instances.
  ...
  protected List getTransientCompanyModelInstancesAsList(String[] beanNames) {
    return new ArrayList(Arrays.asList(getTransientCompanyModelInstances(beanNames)));
  }
  ...
  protected void runSimplePrimitiveTypesQuery(
      String filter, PersistenceManager pm, Collection expected, String assertion) {
    Query q = pm.newQuery();
  ...
  protected void printOutput(Object results, Collection expected) {
    ...
    if (!(results instanceof Collection)) {
      logger.debug("Query result is not a collection: " + results.getClass().getName());
    }
