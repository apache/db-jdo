 ...
 * <B>Keywords:</B> getObjectIds getObjectId <br>
 * <B>Assertion IDs:</B> A8.3-11 <br>
 * <B>Assertion Description: </B> These methods return the JDO identities of the parameter
 * instances. For each instance in the parameter, the getObjectId method is called. They return one
 * identity instance for each persistence-capable instance in the parameter. The order of iteration
 * of the returned Collection exactly matches the order of iteration of the parameter Collection.]
 */
public class GetObjectIds extends JDO_Test {
  ...
  protected void localSetUp() {
    ...
    singleFieldIdentityObjs =
        new PCPointSingleFieldInteger[] {
    ...
    singleFieldIdentityOids =
        new SingleFieldIdentity[] {
          singleFieldIdentityOid0 =
              (SingleFieldIdentity) JDOHelper.getObjectId(singleFieldIdentityObj0),
          singleFieldIdentityOid1 =
              (SingleFieldIdentity) JDOHelper.getObjectId(singleFieldIdentityObj1)
        };
  }
  ...
  public void testGetObjectIdsArray() {
    ...
    for (int i = 0; i < objs.length; ++i) {
      ...
      if (expected == null ? actual != null : !expected.equals(actual)) {
        messages.append(
            "\nComparison failed for object ids at position "
                + i
                + "\nexpected: "
                + expected
                + "\nactual: "
    ...
    if (messages.length() != 0) {
      fail(ASSERTION_FAILED + "getObjectIds(Object[] pcs) " + messages.toString());
    }
  }
  ...
  public void testGetObjectIdsCollection() {
    ...
    if (messages.length() != 0) {
      fail(ASSERTION_FAILED + "getObjectIds(Collection pcs) " + messages.toString());
    }
  }
  ...
  public void testTypedGetObjectIdsCollection() {
    ...
    if (messages.length() != 0) {
      fail(
          ASSERTION_FAILED
              + "getObjectIds(Collection<SingleFieldIdentyty> pcs) "
              + messages.toString());
    }
  }
}
