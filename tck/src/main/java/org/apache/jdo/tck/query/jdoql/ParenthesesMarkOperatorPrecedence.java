...
/**
 * <B>Title:</B> Parentheses Mark Operator Precedence <br>
 * <B>Keywords:</B> query <br>
 * <B>Assertion ID:</B> A14.6.2-37. <br>
...
public class ParenthesesMarkOperatorPrecedence extends QueryTest {
  ...
  void runTestParenthesesMarkOperatorPrecedence01(PersistenceManager pm) {
    ...
    try {
      ...
      checkQueryResultWithoutOrder(ASSERTION_FAILED, "x == (1 + 1)", results, expected);
      if (debug) logger.debug("Test ParenthesesMarkOperatorPrecedence01 - Passed");
  ...
  void runTestParenthesesMarkOperatorPrecedence02(PersistenceManager pm) {
    ...
    try {
      ...
      expected = getFromInserted(expected);
      printOutput(results, expected);
      checkQueryResultWithoutOrder(ASSERTION_FAILED, "x == (1 + 1) * 2", results, expected);
      if (debug) logger.debug("Test ParenthesesMarkOperatorPrecedence02 - Passed");
  ...
  void runTestParenthesesMarkOperatorPrecedence03(PersistenceManager pm) {
    ...
    try {
      ...
      checkQueryResultWithoutOrder(
          ASSERTION_FAILED, "x == (9 - 1) * (3 + 5) / 32", results, expected);
      if (debug) logger.debug("Test ParenthesesMarkOperatorPrecedence03 - Passed");
  ...
  void runTestParenthesesMarkOperatorPrecedence04(PersistenceManager pm) {
    ...
    try {
      ...
      expected = getFromInserted(expected);
      printOutput(results, expected);
      checkQueryResultWithoutOrder(ASSERTION_FAILED, "x == 2 && y == 2", results, expected);
      if (debug) logger.debug("Test ParenthesesMarkOperatorPrecedence04 - Passed");
  ...
  void runTestParenthesesMarkOperatorPrecedence05(PersistenceManager pm) {
    ...
    try {
      ...
      printOutput(results, expected);
      checkQueryResultWithoutOrder(
          ASSERTION_FAILED, "x == (1 + 1) || y == (1002 - 1000)", results, expected);
      if (debug) logger.debug("Test ParenthesesMarkOperatorPrecedence05 - Passed");
  ...
  void runTestParenthesesMarkOperatorPrecedence06(PersistenceManager pm) {
    ...
    try {
      ...
      expected = getFromInserted(expected);
      printOutput(results, expected);
      checkQueryResultWithoutOrder(ASSERTION_FAILED, "x == (1 * 2) && true", results, expected);
      if (debug) logger.debug("Test ParenthesesMarkOperatorPrecedence06 - Passed");
  ...
  void runTestParenthesesMarkOperatorPrecedence07(PersistenceManager pm) {
    ...
    try {
      ...
      printOutput(results, expected);
      checkQueryResultWithoutOrder(
          ASSERTION_FAILED, "x == (10000 / 5000) || false", results, expected);
  ...
  void runTestParenthesesMarkOperatorPrecedence08(PersistenceManager pm) {
    ...
    try {
      ...
      expected = getFromInserted(expected);
      printOutput(results, expected);
      checkQueryResultWithoutOrder(ASSERTION_FAILED, "(x == 2) == true", results, expected);
  ...
  void runTestParenthesesMarkOperatorPrecedence09(PersistenceManager pm) {
    ...
    try {
      ...
      printOutput(results, expected);
      checkQueryResultWithoutOrder(
          ASSERTION_FAILED, "(x == ((21 - 1/1)/10 + 1)) | (false && true)", results, expected);
      if (debug) logger.debug("Test ParenthesesMarkOperatorPrecedence09 - Passed");
