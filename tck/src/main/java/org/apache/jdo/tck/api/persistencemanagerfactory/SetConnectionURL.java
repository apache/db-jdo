 ...
 * <B>Assertion IDs:</B> A11.1-16,A11.1-17. <br>
 * <B>Assertion Description: </B> PersistenceManagerFactory.setConnectionURL(String URL) sets the
 * value of the ConnectionURL property (the URL for the data source).
...
public class SetConnectionURL extends JDO_Test {
  ...
  public void test() {
    try {
      if (url == null) {
        throw new JDOException("Missing PMF property " + CONNECTION_URL_PROP);
      }
      pmf.setConnectionURL(url);
      if (!url.equals(pmf.getConnectionURL())) {
        fail(
            ASSERTION_FAILED,
            "ConnectionURL set to '"
                + url
                + "' ,"
