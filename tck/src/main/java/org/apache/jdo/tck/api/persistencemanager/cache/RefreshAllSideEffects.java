...
public class RefreshAllSideEffects extends PersistenceManagerTest {
  ...
  private void runTestRefreshAllSideEffects(boolean doRefresh) {
    ...
    try {
      ...
      if (doRefresh) {
        appendMessage(
            "Expected no exception on commit with doRefresh " + "true, but got " + ove.toString());
      }
      // else expect exception
    } catch (Exception e) {
      appendMessage(
          "Unexpected exception on commit. doRefresh is "
              + doRefresh
              + ".  Exception is: "
              + e.toString());
    }
    ...
    txVerify.begin();
    Collection pointsVerify = pmVerify.getObjectsById(oids, true);
