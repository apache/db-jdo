 ...
 * <B>Keywords:</B> identity cache lifecycle <br>
 * <B>Assertion IDs:</B> A12.5.6-3 <br>
 ...
 * the same JDO identity as the oid parameter, there is no change made to the state of the returned
 * instance.
 */
  ...
  public void testGetObjectByIdNoValidationInstanceInCacheNoStateChange() {
    ...
    try {
      ...
      if (actualState != curr) {
        fail(
            ASSERTION_FAILED,
            "State mismatched - Expected state: " + curr + " Actual state: " + actualState);
      }
      tx.commit();
      tx = null;
    } finally {
