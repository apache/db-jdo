 ...
 * <B>Assertion IDs:</B> A12.10-2 <br>
 * <B>Assertion Description: </B> A call to PersistenceManager.getObjectIdClass returns null if the
 * class is abstract or not persistence-capable, or the parameter is null.
 */
public class GetObjectIdClassForAbstractOrNonPersistenceCapableClass
  ...
  private void runTestNonPcClass(PersistenceManager pm) {
    ...
    if (c1 != null) {
      fail(
          ASSERTION_FAILED, "pm.getObjectIdClass returns non-null ObjectId class for non pc class");
    }
  }
  ...
  private void runTestNullParam(PersistenceManager pm) {
    ...
    if (c1 != null) {
      fail(
          ASSERTION_FAILED,
          "pm.getObjectIdClass for null returned non-null class: " + c1.getName());
    }
  }
}
