 ...
 * <B>Keywords:</B> <br>
 * <B>Assertion ID:</B> A11.3-3. <br>
 * <B>Assertion Description: </B> After the first use of <code>
 * PersistenceManagerFactory.getPersistenceManager()</code>, none of the <code>set</code> methods
 * will succeed.
 */
public class AfterGetPersistenceManagerNoSetMethodsSucceed extends JDO_Test {
  ...
  public void runTest(boolean bUserAndPasswd) {
    pmf = getPMF();
    if (!bUserAndPasswd) pm = getPM();
    else pm = getPMF().getPersistenceManager(username, password);
    ...
    for (Iterator it = setCollection.iterator(); it.hasNext(); ) {
      ...
      try {
        ...
        if (debug) logger.debug("Caught expected exception " + ex.toString() + " from " + where);
      }
    }
    ...
    for (Iterator it = getCollection.iterator(); it.hasNext(); ) {
      ...
      try {
        ...
        fail(ASSERTION_FAILED, "Caught unexpected exception " + ex.toString() + " from " + where);
      }
    }
  }
  ...
  static class GetProperty {
    ...
    GetProperty(String methodName) {
      this.methodName = methodName;
      try {
        method = PersistenceManagerFactory.class.getMethod(methodName, (Class[]) null);
      } catch (NoSuchMethodException ex) {
        throw new JDOFatalInternalException("Method not defined: " + methodName);
      }
    }
