...
/**
 * <B>Title:</B> Test that a PM returns the same JDO instance if getObjectById(ObjectId) is called
 * multiple times with the same objectId instance <br>
 * <B>Keywords:</B> lifecycle getObjectById <br>
 * <B>Assertion IDs:</B> A5.4-2, A5.4-8 <br>
 * <B>Assertion Description: </B> Any instances returned by any calls to getObjectById(Object) with
 * the same object id instance to the same PersistenceManager instance must be identical (assuming
 * the instances were not garbage collected between calls).
 */
  ...
  public void test() {
    ...
    try {
      ...
      for (int i = 0; i < NUM_OBJECTS; i++) {
        ...
        if (!(obj2 == obj[i])) {
          fail(ASSERTION_FAILED, "objects with OId = " + objId + " are not identical");
        }
      }
