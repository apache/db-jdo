 ...
 * <B>Keywords:</B> embedded lifecycle <br>
 ...
 * made to the instance are are reflected as changes to the datastore per transaction requirements.
 */
public class SecondClassObjectsTrackTheirChanges extends JDO_Test {
    ...
    if ((currComp != PERSISTENT_DIRTY) && (currAddr != PERSISTENT_DIRTY)) {
      fail(
          ...
          "Unable to create persistent-dirty instance "
              ...
              + "state of Company instance is "
              + states[currComp]
              + " and state of Address instance is "
              + states[currAddr]);
    }
  }
    ...
    if ((currComp != PERSISTENT_DIRTY) && (currDepts != PERSISTENT_DIRTY)) {
      fail(
          ...
          "Unable to create persistent-dirty instance "
              ...
              + states[currComp]
              + " and state of Departments instance is "
              + states[currDepts]);
    }
  }
  ...
  public Company getPersistentNewInstance(long companyid) {
    Company obj =
        new Company(companyid, "MyCompany", new Date(), new Address(0, "", "", "", "", ""));
