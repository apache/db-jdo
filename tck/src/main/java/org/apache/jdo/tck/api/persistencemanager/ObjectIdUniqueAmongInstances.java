...
/**
 * <B>Title:</B> ObjectId Unique Among Instances <br>
 * <B>Keywords:</B> identity <br>
 * <B>Assertion ID:</B> A12.5.6-12. <br>
 * <B>Assertion Description: </B> Within a transaction, the <code>ObjectId</code> returned will
 * compare equal to the <code>ObjectId</code> returned by only one among all JDO instances
 * associated with the <code>PersistenceManager</code> regardless of the type of <code>ObjectId
 * </code>.
 */
  ...
  public void test() {
    ...
    try {
      ...
      if (oids.size() != instances.size()) {
        fail(ASSERTION_FAILED, "Oids has size: " + oids.size() + "; expected: " + instances.size());
      }
    } finally {
