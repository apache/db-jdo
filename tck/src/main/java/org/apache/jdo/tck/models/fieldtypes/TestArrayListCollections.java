...
/**
 * <B>Title:</B> Support of field type ArrayList. <br>
 * <B>Keywords:</B> model <br>
 * <B>Assertion ID:</B> A6.4.3-23. <br>
 * <B>Assertion Description: </B> If the ArrayList optional feature is supported, then JDO
 * implementation must support fields of the mutable object class <code>ArrayList</code>, supporting
...
public class TestArrayListCollections extends JDO_Test {
  ...
  private void checkValues(Object oid, ArrayListCollections expectedValue) {
    ...
    for (i = 0; i < n; ++i) {
      ...
      if (actual == null) {
        sbuf.append("\nFor element " + i + ", retrieved instance is null");
        continue;
      }
      if (actual.size() != expected.size()) {
        ...
        if (TestUtil.getFieldSpecs(ArrayListCollections.fieldSpecs[i]).equals("BigDecimal")) {
          ...
          for (int j = 0; j < actualL.size(); ++j) {
            BigDecimal bigDecCompareWith = (BigDecimal) expectedL.get(j);
