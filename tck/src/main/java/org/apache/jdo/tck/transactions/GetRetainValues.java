 ...
 * <B>Assertion ID:</B> A13.4.2-17. <br>
 * <B>Assertion Description: </B> Transaction.getRetainValues returns the currently active setting
 * for the RetainValues flag.
 */
  ...
  public void runTestGetRetainValues(PersistenceManager pm) {
    ...
    try {
      ...
      if (!tx.getRetainValues()) {
        fail(ASSERTION_FAILED, "tx.getRetainValues returns false after setting the flag to true.");
      }
      tx.commit();
      if (!tx.getRetainValues()) {
        fail(ASSERTION_FAILED, "tx.getRetainValues returns false after setting the flag to true.");
      ...
      tx.begin();
      if (tx.getRetainValues()) {
        fail(ASSERTION_FAILED, "tx.getRetainValues returns true after setting the flag to false.");
      }
      tx.commit();
      if (tx.getRetainValues()) {
        fail(ASSERTION_FAILED, "tx.getRetainValues returns true after setting the flag to false.");
