 ...
 * <B>Assertion IDs:</B> A12.5.7-24 <br>
 * <B>Assertion Description: </B> PersistenceManager.makeTransactional and makeTransactionalAll can
 * be used to mark a nontransactional persistent instance as being part of the read-consistency set
 * of the transaction. In this case, the call must be made in the context of an active transaction,
 * or a JDOUserException is thrown. To test this, get an instance in the
 * persistence-nontransactional state, make it transactional, then in a different transaction commit
 * a change to the instance. An exception should then be thrown when the first transaction commits.
 */
public class MakeTransactionalANontransactionalPersistentInstance extends PersistenceManagerTest {
  ...
  public boolean makePersistentNonTranInstance(PCPoint obj) {
    ...
    if (curr != PERSISTENT_NONTRANSACTIONAL && curr != HOLLOW) {
      fail(
          ASSERTION_FAILED,
          "expected P-HOLLOW or P-NONTX instance, instance "
              + obj
              + " is "
