...
public class BatchTestRunner extends TestRunner {
   ...
   * of the returned instanec. The class must extend junit.textui.ResultPrinter.
   *
      ...
      if (className.length() != 0) {
        ...
        try {
          ...
          if (!Boolean.getBoolean("no.log.file")) {
            try {
              ctor = clazz.getConstructor(new Class[] {ConsoleFileOutput.class});
          ...
          if (ctor == null) {
            ctor = clazz.getConstructor(new Class[] {PrintStream.class});
            stream = System.out;
          }
