...
/**
 * <B>Title:</B> Support of field type Boolean. <br>
 * <B>Keywords:</B> model <br>
 * <B>Assertion ID:</B> A6.4.3-9. <br>
 * <B>Assertion Description: </B> JDO implementations must support fields of the immutable object
 * class <code>java.lang.Boolean</code>, and may choose to support them as Second Class Objects or
 * First Class Objects.
 */
public class TestFieldsOfBoolean extends JDO_Test {
  ...
  private void checkValues(Object oid, Boolean startValue) {
    ...
    int n = pi.getLength();
    for (i = 0; i < n; ++i) {
      if (!FieldsOfBoolean.isPersistent[i]) continue;
      Boolean val = pi.get(i);
      if (!val.equals(startValue)) {
        fail(
            ASSERTION_FAILED,
            "Incorrect value for "
                + FieldsOfBoolean.fieldSpecs[i]
                + ", expected value "
                + startValue.toString()
                + ", value is "
                + val);
