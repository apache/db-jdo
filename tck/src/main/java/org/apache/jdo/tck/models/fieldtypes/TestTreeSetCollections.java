...
/**
 * <B>Title:</B> Support of field type TreeSet. <br>
 * <B>Keywords:</B> model <br>
 * <B>Assertion ID:</B> A6.4.3-28. <br>
 * <B>Assertion Description: </B> If the TreeSet optional feature is supported, then JDO
 * implementation must support fields of the mutable object class <code>TreeSet</code>, supporting
...
public class TestTreeSetCollections extends JDO_Test {
  ...
  private void setValues(TreeSetCollections collect, int order) {
    ...
    for (int i = 0; i < n; ++i) {
      String valueType = TestUtil.getFieldSpecs(TreeSetCollections.fieldSpecs[i]);
      value = TestUtil.makeNewVectorInstance(valueType, order);
      TreeSet treeSet = new TreeSet((Collection) value);
      collect.set(i, treeSet);
      if (debug) logger.debug("Set " + i + "th value to: " + treeSet.toString());
    }
  }
