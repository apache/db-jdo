...
public class IgnoreCacheFalse extends QueryTest {
  ...
  void runTestNewInstance(PersistenceManager pm) {
    ...
    List<PrimitiveTypes> result =
        pm.newQuery(PrimitiveTypes.class, "intNotNull == 98").executeList();
    // check result
    if (result.isEmpty())
  ...
  void runTestDeletedInstance(PersistenceManager pm) {
    ...
    List<PrimitiveTypes> tmp = pm.newQuery(PrimitiveTypes.class, "id == 3").executeList();
    PrimitiveTypes instance3 = tmp.iterator().next();
    pm.deletePersistent(instance3);

    List<PrimitiveTypes> result =
        pm.newQuery(PrimitiveTypes.class, "intNotNull == 3").executeList();
    // check result
    if (result.isEmpty()) {
  ...
  void runTestModifiedInstance(PersistenceManager pm) {
    ...
    List<PrimitiveTypes> tmp = pm.newQuery(PrimitiveTypes.class, "id == 5").executeList();
    PrimitiveTypes instance5 = tmp.iterator().next();
    instance5.setIntNotNull(99);

    List<PrimitiveTypes> result =
        pm.newQuery(PrimitiveTypes.class, "intNotNull == 99").executeList();
    // check result
    if (result.isEmpty()) {
