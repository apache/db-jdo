 ...
 * <B>Assertion Description: </B> With ignoreCache set to false:
 *
 * <UL>
 *   <LI>if instances were made persistent in the current transaction, the instances will be
 *       considered part of the candidate instances.
 *   <LI>if instances were deleted in the current transaction, the instances will not be considered
 *       part of the candidate instances.
 *   <LI>modified instances will be evaluated using their current transactional values.
 * </UL>
 */
public class IgnoreCacheFalse extends QueryTest {
  ...
  void runTestNewInstance(PersistenceManager pm) {
    ...
    Collection result =
        (Collection) pm.newQuery(PrimitiveTypes.class, "intNotNull == 98").execute();
    // check result
    if (result.isEmpty())
    ...
    if (p.getId() != 98L)
      fail(ASSERTION_FAILED, "Query returned wrong instance with id " + p.getId());
    if (i.hasNext()) fail(ASSERTION_FAILED, "Query returned more than one instance");

    if (debug) logger.debug("New instance is part of query result");
  ...
  void runTestDeletedInstance(PersistenceManager pm) {
    ...
    Collection tmp = (Collection) pm.newQuery(PrimitiveTypes.class, "id == 3").execute();
    PrimitiveTypes instance3 = (PrimitiveTypes) tmp.iterator().next();
    pm.deletePersistent(instance3);

    Collection result = (Collection) pm.newQuery(PrimitiveTypes.class, "intNotNull == 3").execute();
    // check result
    if (result.isEmpty()) {
      if (debug) logger.debug("deleted instance not part of query result.");
    } else {
      ...
      else fail(ASSERTION_FAILED, "query returns suspicious isntance " + p);
    }
    tx.rollback();
  }
  ...
  void runTestModifiedInstance(PersistenceManager pm) {
    ...
    Collection tmp = (Collection) pm.newQuery(PrimitiveTypes.class, "id == 5").execute();
    PrimitiveTypes instance5 = (PrimitiveTypes) tmp.iterator().next();
    instance5.setIntNotNull(99);

    Collection result =
        (Collection) pm.newQuery(PrimitiveTypes.class, "intNotNull == 99").execute();
    // check result
    if (result.isEmpty()) {
    ...
    if (p.getIntNotNull() != 99)
      fail(
          ASSERTION_FAILED,
          "Query returned instance with wrong intNotNull field value " + p.getIntNotNull());
    if (i.hasNext()) fail(ASSERTION_FAILED, "Query returned more than one instance");

    if (debug) logger.debug("Modified instance is part of query result");
