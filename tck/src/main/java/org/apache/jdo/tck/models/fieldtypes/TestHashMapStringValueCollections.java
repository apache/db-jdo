...
/**
 * <B>Title:</B> Support of field type HashMap, varying key type <br>
...
public class TestHashMapStringValueCollections extends JDO_Test {
  ...
  void runTest(PersistenceManager pm) {
    ...
    HashMapStringValueCollections expectedValue = new HashMapStringValueCollections();
  ...
  private void setValues(HashMapStringValueCollections collect, int order) {
    ...
    for (int i = 0; i < n; ++i) {
      Vector fieldSpecs = TestUtil.getFieldSpecsForMap(HashMapStringValueCollections.fieldSpecs[i]);
  ...
  /** */
  private void checkValues(Object oid, HashMapStringValueCollections expectedValue) {
    StringBuffer sbuf = new StringBuffer();
    HashMapStringValueCollections pi = (HashMapStringValueCollections) pm.getObjectById(oid, true);
    int n = pi.getLength();
    for (int i = 0; i < n; ++i) {
