 ...
 * <B>Keywords:</B> addLifeCycleListener removeLifeCycleListener <br>
 * <B>Assertion IDs:</B> A12.15-1 <br>
 ...
 * after the instance transitions to persistent-new.
 */
public class InstanceLifecycleListenerCreate extends AbstractInstanceLifecycleListener {
  ...
  private final InstanceLifecycleListenerImpl listener = new InstanceLifecycleListenerCreateImpl();
  ...
  public void testCreate() {
    ...
    listener.verifyCallbacks(
        ASSERTION1_FAILED, new int[] {InstanceLifecycleListenerImpl.POST_CREATE_LISTENER});
  }

  /** The LifeCycleListener class to be registered with the PersistenceManager. */
  private static class InstanceLifecycleListenerCreateImpl extends InstanceLifecycleListenerImpl {

    @Override
    public void postCreate(InstanceLifecycleEvent event) {
      notifyEvent(POST_CREATE_LISTENER);
      checkEventType(ASSERTION1_FAILED, InstanceLifecycleEvent.CREATE, event.getEventType());
      checkEventSource(ASSERTION1_FAILED, event.getSource());
      checkPersistent(ASSERTION1_FAILED + " in postCreate, ", expectedSource);
      checkNew(ASSERTION1_FAILED + " in postCreate, ", expectedSource);
    }
  }
}
