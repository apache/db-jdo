...
/**
 * <B>Title:</B> Support of field type Map, varying value type <br>
...
public class TestMapStringKeyCollections extends JDO_Test {
  ...
  private void setValues(MapStringKeyCollections collect, int order) {
    ...
    for (int i = 0; i < n; ++i) {
      Vector<String> fieldSpecs =
          TestUtil.getFieldSpecsForMap(MapStringKeyCollections.fieldSpecs[i]);
  ...
  /** */
  private void checkValues(Object oid, MapStringKeyCollections expectedValue) {
    ...
    for (int i = 0; i < n; ++i) {
      ...
      if (actual.size() != expected.size()) {
        ...
        if (TestUtil.getFieldSpecsForMap(MapStringKeyCollections.fieldSpecs[i])
            .get(1)
            .equals("BigDecimal")) {
