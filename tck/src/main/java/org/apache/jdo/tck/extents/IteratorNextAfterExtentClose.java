...
/**
 * <B>Title:</B> Iterator Next After Extent Close <br>
 * <B>Keywords:</B> extent exception <br>
 * <B>Assertion ID:</B> A15.3-10. <br>
 * <B>Assertion Description: </B> After a call to <code>Extent.close(Iterator i)</code>, the
 * parameter <code>Iterator</code> will throw a <code>NoSuchElementException</code> to <code>next()
 * </code>.
 */
  ...
  public void test() {
    ...
    try {
      ...
      try {
        ...
        if (count4 != 2) {
          fail(ASSERTION_FAILED, "Iterator4 after rollback: " + count4 + "; should be 2");
        }
      } catch (Exception unexpected) {
        fail(ASSERTION_FAILED, "unexpected exception " + unexpected);
      }
    } catch (Exception unexpected) {
      fail(ASSERTION_FAILED, "unexpected exception " + unexpected);
    }
