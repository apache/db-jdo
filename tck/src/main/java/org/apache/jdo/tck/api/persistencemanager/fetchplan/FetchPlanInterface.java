 ...
 * <B>Assertion IDs:</B> 12.7.1-1 <br>
 * <B>Assertion Description: </B> public interface FetchPlan { String DEFAULT = "default"; String
 * ALL = "all"; int FETCH_SIZE_GREEDY = -1; int FETCH_SIZE_OPTIMAL = 0; int DETACH_LOAD_FIELDS = 1;
 * int DETACH_UNLOAD_FIELDS = 2; A12.7.1-1 [/** Add the fetchgroup to the set of active fetch
 * groups. Duplicate names will be removed. FetchPlan addGroup(String fetchGroupName); /** Remove
 * the fetch group from the set active fetch groups. FetchPlan removeGroup(String fetchGroupName);
 * /** Remove all active groups, including the default fetch group. FetchPlan clearGroups(); /**
 * Return an immutable Set of the names of all active fetch groups. Set getGroups(); /** Set a
 * Collection of group names to replace the current groups. Duplicate names will be removed.
 * FetchPlan setGroups(Collection fetchGroupNames); /** Set an array of group names to replace the
 * current groups. Duplicate names will be removed. FetchPlan setGroups(String[] fetchGroupNames);
 * /** Set a single group to replace the current groups. FetchPlan setGroup(String fetchGroupName);]
 * /** Set the roots for DetachAllOnCommit FetchPlan setDetachmentRoots(Collection roots); /** Get
 * the roots for DetachAllOnCommit Collection getDetachmentRoots(); /** Set the roots for
 * DetachAllOnCommit FetchPlan setDetachmentRootClasses(Class[] rootClasses); /** Get the roots for
 * DetachAllOnCommit Class[] getDetachmentRootClasses(); /** Set the maximum fetch depth. FetchPlan
 * setMaxFetchDepth(int fetchDepth); /** Get the maximum fetch depth. int getMaxFetchDepth();
 * A12.7.1-2 [/** Set the fetch size for large result set support. FetchPlan setFetchSize(int
 * fetchSize); /** Return the fetch size; 0 if not set; -1 for greedy fetching. int getFetchSize();]
 * A12.7.1-3 [/** Set detachment options FetchPlan setDetachmentOptions(int options); /** Return the
 * detachment options int getDetachmentOptions();]
 */
public class FetchPlanInterface extends JDO_Test {
   ...
   */
  protected <T> boolean setEquals(Collection<T> expected, Collection<T> actual) {
   ...
   * @param actual actual value
   */
  protected void failCompare(String message, Object expected, Object actual) {
   ...
   * @param actual actual value
   */
  protected void failCompare(String message, int expected, int actual) {
    appendMessage(ASSERTION_FAILED + message);
    appendMessage("expected: " + expected);
    appendMessage("actual: " + actual);
  }
  ...
  public void testMaxFetchDepth() {
    ...
    if (expectedMaxFetchDepth != actualMaxFetchDepth) {
      failCompare(
          "testMaxFetchDepth(): wrong getMaxFetchDepth() " + "after setMaxFetchDepth().",
          expectedMaxFetchDepth,
          actualMaxFetchDepth);
    }
    cleanupPM();
    failOnError();
  ...
  public void testFetchSize() {
    ...
    if (expectedFetchSize != actualFetchSize) {
      failCompare(
          "testFetchSize(): wrong getFetchSize() " + "after setFetchSize().",
          expectedFetchSize,
          actualFetchSize);
    }
    cleanupPM();
  ...
  public void checkDefaultGroups() {
    ...
    if (!setEquals(expectedGroups, groups)) {
      failCompare(
          "checkDefaultGroups(): wrong getGroups() " + "after getPersistenceManager().",
          expectedGroups,
          groups);
    }
    cleanupPM();
  ...
  public void checkClearGroups() {
    ...
    if (!setEquals(expectedGroups, groups)) {
      failCompare(
          "checkClearGroups(): wrong getGroups() " + "after clearGroups.", expectedGroups, groups);
    }
    cleanupPM();
  }
  ...
  public void checkSetGroup() {
    ...
    if (!setEquals(expectedGroups, groups)) {
      failCompare(
          "checkSetGroup(): wrong getGroups() " + "after setGroup.", expectedGroups, groups);
    }
    cleanupPM();
  }
  ...
  public void checkAddGroup() {
    ...
    if (!setEquals(expectedGroups, groups)) {
      failCompare(
          "checkAddGroup(): wrong getGroups() " + "after addGroup.", expectedGroups, groups);
    }
    cleanupPM();
  }
  ...
  public void checkRemoveGroup() {
    ...
    if (!setEquals(expectedGroups, groups)) {
      failCompare(
          "checkRemoveGroup(): wrong getGroups() " + "after removeGroup.", expectedGroups, groups);
    }
    cleanupPM();
  }
  ...
  public void checkSetGroupsCollection() {
    ...
    if (!setEquals(expectedGroups, groups)) {
      failCompare(
          "checkSetGroupsCollection(): wrong getGroups() " + "after SetGroups(Collection).",
          expectedGroups,
          groups);
    }
    cleanupPM();
  ...
  public void checkSetGroupsArray() {
    ...
    if (!setEquals(expectedGroups, groups)) {
      failCompare(
          "checkSetGroupsArray(): wrong getGroups() " + "after setGroups(String[]).",
          expectedGroups,
          groups);
    }
    cleanupPM();
  ...
  protected void checkSetDetachmentRoots() {
    ...
    if (!setEquals(expectedRoots, roots)) {
      failCompare(
          "checkGetDetachmentRoots(): wrong getDetachmentRoots() " + "after setDetachmentRoots().",
          expectedRoots,
          roots);
    }
    cleanupPM();
  ...
  private void checkSetDetachmentRootClasses() {
    ...
    FetchPlan fp = getPM().getFetchPlan();
    fp.setDetachmentRootClasses(expectedRootClasses);
