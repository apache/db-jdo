 ...
 * <B>Keywords:</B> LifeCycleListener preStore postStore <br>
 * <B>Assertion IDs:</B> A12.15-3 A12.15-4 <br>
 * <B>Assertion Description: </B> A12.15-3 void preStore(InstanceLifecycleEvent event); This method
 * is called whenever a persistent instance is stored, for example during flush or commit. It is
 * called before the jdoPreStore method is invoked on the instance. A12.15-4 void
 ...
 * instance when this callback is invoked.
 */
public class InstanceLifecycleListenerStore extends AbstractInstanceLifecycleListener {

  /** The InstanceLifecycleListener used for this test */
  private final InstanceLifecycleListenerImpl listener = new InstanceLifecycleListenerStoreImpl();
  ...
  /**
   * This test creates a new persistent instance and commits it. The store callback and listeners
   * will be called.
   */
  ...
  private static class InstanceLifecycleListenerStoreImpl extends InstanceLifecycleListenerImpl {

    public void preStore(InstanceLifecycleEvent event) {
      notifyEvent(PRE_STORE_LISTENER);
      checkEventType(ASSERTION3_FAILED, InstanceLifecycleEvent.STORE, event.getEventType());
      checkEventSource(ASSERTION3_FAILED, event.getSource());
      checkPersistent(ASSERTION3_FAILED + " in preStore, ", expectedSource);
    }

    public void postStore(InstanceLifecycleEvent event) {
      notifyEvent(POST_STORE_LISTENER);
      checkEventType(ASSERTION4_FAILED, InstanceLifecycleEvent.STORE, event.getEventType());
      checkEventSource(ASSERTION4_FAILED, event.getSource());
      checkPersistent(ASSERTION4_FAILED + " in postStore, ", expectedSource);
    }
  }
