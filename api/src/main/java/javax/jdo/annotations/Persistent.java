...
/**
 * Annotation for defining the persistence of a member. This corresponds to the xml elements "field"
 * and "property".
 *
 * @version 2.1
 * @since 2.1
...
public @interface Persistent {
  /**
   * Modifier for this field. This is normally not specified, and the defaults are used, or
   * the @Transactional or @NotPersistent annotation is specified directly on the member. One
   * possible use for specifying persistenceModifier is for embedded instances in which a member is
   * not persistent but in the non-embedded instances the member is persistent. Note that it is not
   * portable to specify a member to be not persistent in the non-embedded case and persistent in
   * the embedded usage.
   *
  ...
  /**
   * Whether related object(s) of this member are dependent and so deleted when this object is
   * deleted.
   *
   * @return whether the related object(s) of this member are dependent
   */
  ...
  /**
   * Whether this member is part of the primary key for application identity. This is equivalent to
   * specifying @PrimaryKey as a separate annotation on the member.
   *
  ...
  /**
   * Custom value strategy to use to generate the value for this field or property (if any). If
   * customValueStrategy is non-empty, then valueStrategy must be UNSPECIFIED.
   *
  ...
  /**
   * Name of the fetch-group to use when this member is loaded due to being referenced when not
   * already loaded.
   *
  ...
  /**
   * Types of the member. Used when the declared member type is a supertype of the actual type that
   * is stored in the member. For example, the declared member type might be an interface type that
   * must contain an object of a concrete type when used for persistence.
   *
   * @return the types
   */
  Class[] types() default {};
  ...
  /**
   * Name of the related member in the other class where this value is mapped (bidirectional
   * relationship).
   *
  ...
  /**
   * Column definition(s) for this member. Used for mapping multiple columns to the same member, for
   * example relationships with multiple column foreign keys.
   *
  ...
  /**
   * Null indicator column for this member. Used for nested embedded fields or properties to
   * indicate whether the embedded instance should have a null value.
   *
  ...
  /**
   * Name of the member when this is embedded in another object. The fully-qualified member name is
   * used. For example, "line.point1.x" refers to the member x in class Point that is embedded as
   * member point1 in class Line that is embedded in a member called line.
   *
  ...
  /**
   * Recursion depth for this member. Used only when the annotation is used within the definition of
   * a FetchGroup.
   *
   ...
   *
   * @return Converter class for converting this member when not PersistenceCapable (or
