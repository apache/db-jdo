...
public class InstallSchema extends AbstractTCKMojo {
        ...
        for (String db : dbs) {
            ...
            for (String idtype : idtypes) {
                for (String mapping : mappings) {
                    ...
                    String outFileName = logsDirectory + File.separator + "database" +
                            File.separator + db + "_" + idtype + "_" + mapping + ".txt";
                    String sqlFileName = sqlDirectory + File.separator + db
    ...
    protected void loadSQLFileUsingIJ(String db, String dbDirName, String sqlFileName, String outFileName)
        ...
        try {
            ...
            System.out.println("FAILED!");
            throw new MojoExecutionException(
                    "*> Failed to execute ij: " + ioex.getLocalizedMessage());
        }
    }
    ...
    protected void loadSQLFileUsingJDBC(String db, String dbDirName,
        ...
        try {
            ...
            if (url == null || "".equals(url.trim())) {
                throw new MojoExecutionException("*> connection url needs to be specified, see file " + sqlFileName);
            }
            String user = loader.getUser();
            if (user == null || "".equals(user.trim())) {
                throw new MojoExecutionException("*> connection user needs to be specified, see file " + sqlFileName);
            }
            String passwd = loader.getPassword();
            if (passwd == null || "".equals(passwd.trim())) {
                throw new MojoExecutionException("*> connection passwd needs to be specified, see file " + sqlFileName);
            }
            ...
            try (Connection conn = DriverManager.getConnection(url, user, passwd);
                ...
                throw new MojoExecutionException("*> Failed to initialize JDBC connection, " +
                        "url=" + url + " user=" + user + " password=" + passwd + ": " + ex.getLocalizedMessage());
            } catch (IOException ex) {
                throw new MojoExecutionException("*> Failed to write to the outfile " + outFileName + ": " +
                        ex.getLocalizedMessage());
            }
        } catch (IOException ex) {
            throw new MojoExecutionException("*> Failed to load file " + sqlFileName + " " + ex.getLocalizedMessage());
        }
    }
